(function (window) {
  ...
  MirrorFx.prototype._initTilt = function () {
    var self = this;
    this.imgs.side1.style.WebkitTransition = this.imgs.side2.style.transition =
    ...
    this.mousemoveFn = function (ev) {
      requestAnimationFrame(function () {
        ...
        var mousepos = getMousePos(ev),
          ...
          m =
            self.options.layout === "horizontal"
              ? relmousepos.x
              : relmousepos.y,
          tVal =
            m < d / 2
              ? lineEq(
                  self.options.visibleArea,
                  self.options.visibleArea * self.options.tiltFactor,
                  d / 2,
                  0,
                  m
                )
        ...
        self.imgs.side1.style.WebkitTransform = self.imgs.side1.style.transform = self.imgs.side2.style.WebkitTransform = self.imgs.side2.style.transform =
          "translate" +
          (self.options.layout === "horizontal" ? "X" : "Y") +
          "(" +
          (1 - tVal) * 100 +
          "%) rotateZ(" +
          rz +
  ...
  MirrorFx.prototype._animate = function (action, callback) {
    ...
    var opts = {
      ...
      opacity: {
        ...
        duration:
          action === "show"
            ? this.options.duration[action]
            : this.options.duration[action] * 0.5,
        easing: this.options.easing[action],
      },
      rotateZ: 0,
    };
    ...
    if (this.options.layout === "horizontal") {
      opts.translateX =
        action === "show"
          ? ["100%", Math.ceil((1 - this.options.visibleArea) * 100) + "%"]
          : "100%";
    } else {
      opts.translateY =
        action === "show"
